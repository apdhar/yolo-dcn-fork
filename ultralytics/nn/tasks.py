...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3) 模型配置文件；输出通道数列表，初始为输入通道数，即3
    ...
    ch = [ch]
    layers, save, c2 = [], [], ch[-1]  # layers 模型（模块列表）, savelist, ch out 输出通道数列表，初始为输入通道数3
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (SwinTransformer, Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
            ...
            args = [c1, c2, *args[1:]]  # 转换后的参数列表
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,
                     C2f_SE,
                     C2f_GCS, C2f_GCS05, C2f_GCSCAM, DSConv_C2f, C2f_DCN, C2f_DCN2):
                args.insert(2, n)  # 插入内部结构重复次数参数
                n = 1  # n的两重含义：内部结构重复次数，或者模块重复次数
        elif m is AIFI:
            args = [ch[f], *args]
        ...
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module 模块定义
